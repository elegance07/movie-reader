...
function en_serialize($value)
{
    return str_replace('"', "'", serialize($value));
}

function de_serialize($value)
{
    return unserialize(str_replace("'", '"', $value));
}
...
function clean_text($txt)
{
    ...
    if (in_array("quotes", $_clean_text)) {
        $txt = str_replace(array("\'", "'", '"', "\""), "", $txt);
    }
    if (in_array("hyperlinks", $_clean_text)) {
        $txt = $txt; /* NOT COMPLETED */
    }
    if (in_array("spaces", $_clean_text)) {
        $txt = str_replace(array("\s\s", "\t"), "", $txt);
    }
    if (in_array("breaklines", $_clean_text)) {
...
function download_img($url)
{
    ...
    fclose($fp);
    unset($fp, $output);
    $id = $db->super_query("SELECT MAX(id) as max FROM " . PREFIX . "_post");
    $id['max']++;
    $db->free();
    $id_max = $id['max'];
    $_save = str_replace("posts/", "", $save);
...
function downCover($url, $which = "")
{
    ...
    if (filter_var($url, FILTER_VALIDATE_URL) !== False) {
        ...
        if ($_ext == "jpg" || $_ext == "jpeg" || $_ext == "png") {
            if (substr(bin2hex($output), 0, 20) != "ffd8ffe000104a464946" && substr(bin2hex($output), 0, 20) != "ffd8ffe1001845786966" && substr(bin2hex($output), 0, 8) != "89504e47") {
        ...
        if (!empty($output)) {
            ...
            if (file_exists(UPLOAD_DIR . $save)) {
                if ($mws_film['overwriteimg']) save_img($save, $output, $id['max']);
                $addwm = True;
                $res = $config['http_home_url'] . "uploads/" . $save;
            } else {
                save_img($save, $output, $id['max']);
                $addwm = True;
                $res = $config['http_home_url'] . "uploads/" . $save;
            }
        } else {
        ...
        if ($addwm == True || ($mws_film['insertwm']) || ($mws_film['resizeimg'])) {
            require_once ENGINE_DIR . '/classes/mws-film/thumb.class.php';
            $img = new thumbnail(UPLOAD_DIR . $save);
...
if (isset($_POST['film_url'])) {
        ...
        if ($mws_film['downcover']) {
            try {
                if ($mws_film['imgsize'] && !empty($results['orgimg'])) {
                    $results['img'] = downCover($results['orgimg'], $which);
                    unset($results['orgimg']);
                } else {
            ...
            foreach ($result['items'] as $item) {
                if (!empty($item['id']['videoId'])) {
                    ...
                    if ($mws_film['trailertag'] == "short") {
                        $_link = explode("?v=", $_link);
                        $_link = $_link[1];
                        $_mlink = explode("?v=", $_mlink);
                        $_mlink = $_mlink[1];
                    }
    ...
    // Dosyanın mevcut olup olmadığını ve okunabilir olup olmadığını kontrol edin
    if (!file_exists($file_path)) {
        error_log("Dosya mevcut değil: $file_path");
        echo "Dosya mevcut değil: $file_path";
        exit;
    }

    if (!is_readable($file_path)) {
        error_log("Dosya okunabilir değil: $file_path");
        echo "Dosya okunabilir değil: $file_path";
        exit;
    }
    ...
    if ($data === false) {
        ...
        echo "Dosya okunamadı: $file_path";
        exit;
    }

    // Yedek dosya oluşturma
    $backup_path = ENGINE_DIR . "/data/xfields.txt_backup_" . md5($data);
    if (file_put_contents($backup_path, $data) === false) {
        error_log("Yedek dosya yazılamadı: $backup_path");
        echo "Yedek dosya yazılamadı: $backup_path";
        exit;
    }
    ...
    if (file_put_contents($file_path, $newdata) === false) {
        error_log("Dosya yazılamadı: $file_path");
        echo "Dosya yazılamadı: $file_path";
        exit;
    }
